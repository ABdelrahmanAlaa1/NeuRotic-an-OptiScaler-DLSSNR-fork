...
foreach ($f in $files) {
    ...
    else { throw "Required release file is missing from build output: $f" }
}

if ($ReleaseIniPath) {
    if (-not (Test-Path -LiteralPath $ReleaseIniPath -PathType Leaf)) {
        throw "Release INI not found: $ReleaseIniPath"
    }

    $resolvedReleaseIni = (Resolve-Path -LiteralPath $ReleaseIniPath).Path
    Copy-Item -LiteralPath $resolvedReleaseIni -Destination "$stage\OptiScaler.ini" -Force
    Write-Host "release ini: $resolvedReleaseIni"
}

Copy-Item -LiteralPath "$root\ALPHA-0.9.4.md" -Destination "$stage\RELEASE NOTES - Alpha 0.9.4.md" -Force
...
if (-not $ReleaseIniPath) {
    $ini = Get-Content $iniPath -Raw
    $ini = $ini -replace '(?im)^(\s*LogToFile\s*=\s*)auto(\s*(?:[;#].*)?)$', '${1}true${2}'
    $ini = $ini -replace '(?im)^(\s*LogLevel\s*=\s*)auto(\s*(?:[;#].*)?)$', '${1}2${2}'
    Set-Content $iniPath $ini -Encoding utf8 -NoNewline
}

$check = Select-String -Path $iniPath -Pattern '^\s*(LogToFile|LogLevel)\s*=' | ForEach-Object { $_.Line.Trim() }
...
$on = Select-String -Path "$stage\OptiScaler.ini" -Pattern '^\s*Enabled\s*=\s*true\s*(?:[;#].*)?$'
...
$proprietaryModel = Get-ChildItem -LiteralPath $stage -Recurse -File |
    Where-Object { $_.Name -ieq "nvngx_dlssnr.dll" }

if ($proprietaryModel) {
    throw "REFUSING: proprietary nvngx_dlssnr.dll entered the release stage"
}

Write-Host "redistribution check: proprietary nvngx_dlssnr.dll is absent"
...
Write-Host ("zip sha256: {0}" -f (Get-FileHash -Algorithm SHA256 -LiteralPath $zip).Hash)
